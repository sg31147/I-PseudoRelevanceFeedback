...
def data_retrieve(config: OmegaConf) -> Data:
    """The data pipeline.
    ...
    with vaex.cache.memory_infinite():  # type: ignore
        ...
        with open( dir/'code_system2code_counts_file.json', 'w') as file:
                json.dump(code_system2code_counts, file)
        schema = pa.schema(
            [
...
def data_eval_pipeline(config: OmegaConf) -> Data:
    """The data pipeline.
    ...
    with vaex.cache.memory_infinite():  # type: ignore
        ...
        # Filter df to exclude rows where TARGET_COLUMN is 'train'
        # Convert TARGET_COLUMN to string type if necessary
        df = df.filter(df['split'] != 'train')
...
def data_predict_pipeline(config: OmegaConf, data: pd.DataFrame = None) -> Data:

    dir = Path(config.dir)
    # Check if file exists and load_model is set to True
    if os.path.exists(dir/'code_system2code_counts_file.json'):
        print('code_system2code_counts already exists')
        with open(dir/'code_system2code_counts_file.json', 'r') as file:
            code_system2code_counts = json.load(file)
        
    else:# Otherwise, generate the data and save it
    ...
    # if you have your own code to predict on api test
    if data is not None:
