...
class WandbCallback(BaseCallback):
    ...
    def on_initialisation_end(self, trainer=None):
        ...
        if trainer.lookups is not None:
            ...
            OmegaConf.save(trainer.config, trainer.experiment_path / "config.yaml") # ไป save ใน temp
        wandb.login(key="6405d5683647f2155bb9dbff5cdedf2374f03962")
        wandb.init(
            config=wandb_cfg,
        ...
        if not trainer.config.debug:
            ...
            trainer.experiment_path = (
                ...
                #Path(os.path.dirname(wandb.run.dir))
            )
    ...
    def log_dict(
        ...
        wandb.log(nested_dict) #ส่ง data ไป 
...
class SaveBestModelCallback(BaseCallback):
    ...
    def on_epoch_end(self, trainer=None): # เก็บ metric  [กลุ่ม train test val] [กลุ่ม all proc diag]
        best_metric = trainer.metric_collections[self.split_name][
            self.target_name
        ].get_best_metric(self.metric_name)
        if self.prev_best is None or best_metric != self.prev_best: #เท่ากันไม่เก้บ
            self.prev_best = best_metric #แทนที่สิ่งที่ดีกว่า 
            trainer.save_checkpoint("best_model.pt") #ชี้กลับไปที่ train นะ
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def on_epoch_end(self, trainer=None):
        ...
        if self.prev_best is None or best_metric > self.prev_best: # ดีกว่า เอาแทนที่
            self.prev_best = best_metric
            self.counter = 0
        else:
            self.counter += 1
            #จุดที่ชี้บ่งว่าจะหลุด train 
